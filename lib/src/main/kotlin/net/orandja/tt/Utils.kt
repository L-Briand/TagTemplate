...
inline fun <reified T : Any> T.toKeyValueGroup() = TT.group(reflectMap().toTemplateValue())
inline fun <reified T : Any> List<T>.toTemplateRoll() = TT.roll(map { it.toKeyValueGroup() })

inline fun <reified T> List<T>.makeTemplateValue(key: String) = map { TT.group(key to TT.value("$it")) }
inline fun <reified T> List<T>.makeTemplate(key: String, action: (T) -> TemplateRenderer) = map { TT.group(key to action(it)) }

inline infix fun <reified T : Any> TemplateRenderer.bindToData(data: T) =
    duplicate() bindTo data.toKeyValueGroup()
